...
car_state = np.zeros((7,), dtype=np.float32)
...
def update_plot(i):
    ...
    scat = plt.scatter(x_plt, y_plt, c=zz, marker="s",edgecolors="none", s = 10, alpha = 0.5, cmap='Reds', vmin=0, vmax=1) 

    
    ###------------------------------###
    ### Plotting pose of ego vehicle ###
    ###------------------------------###
    ...
    scat_car_pose = plt.scatter(car_idx[0][0], car_idx[0][1], c='blue', marker='o', s = 20, alpha = 1, label='car0')
    ...
    ### text update
    text_range = "Lookahead range: [{0},{1}] - [{2},{3}]".format(car_state[0], car_state[1], car_state[2], car_state[3])
    text_final_pose = "Final lookahead pose: [{0},{1}]".format(car_state[4], car_state[5])
    text_prob = "Risk at the pose : {0}".format(car_state[6])

    plt.text(10, 0, text_range, va='center')
    plt.text(10, -20, text_final_pose, va='center')
    plt.text(10, -40, text_prob, va='center')
